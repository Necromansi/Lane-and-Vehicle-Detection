...
def birdeye(img):
    ...
    src = np.float32([[w, h-10],    ### Pixel points of 4 corners from the frame
                      [0, h-10],    
                      [546, 460],   
                      [732, 460]])  
    dst = np.float32([[w, h],       ### Destination points
                      [0, h],       
                      [0, 0],       
                      [w, 0]])      
...
#     mode = 'video'  ### Change this to photo if you want output photos, change to video if you want output video


#     #Outputs video in the same folder.
#     if mode == 'video':

#         clip = VideoFileClip('project_video.mp4').fl_image(process_pipeline)
#         clip.write_videofile('out_video.mp4', audio=False)

#     ### Outputs the photos in output_images folder, and displays each photos
#     elif mode =='photo':
